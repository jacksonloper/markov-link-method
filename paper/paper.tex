...
\begin{itemize}
    ...
    \item Let $\Theta(p,h)=\{q:\ h(y|\ell) = \sum_x p(x|\ell)q(y|x)\}$ denote the set of links which are consistent with $p,h$.  Notice that $q^*\in\Theta(p^*,h^*)$, but there may be other links $q\in\Theta(p^*,h^*)$.  Any such link yields the same distribution on what we can observe, $N^X,N^Y$.  Therefore there is simply no way to use data to distinguish among them.  This is the essence of the nonidentifiability problem mentioned above.  So, instead of trying to estimate this inestimable quantity, we will focus on three other quantities which can be identified from data.  For each fixed value of $p^*,h^*$, we define
    \begin{itemize}
        \item $q_{\mathrm{lo},x,y}(p^*,h^*) \triangleq \min_{q\in \Theta(p^*,h^*)} q(y|x)$
        \item $q_{\mathrm{hi},x,y}(p^*,h^*) \triangleq \max_{q\in \Theta(p^*,h^*)} q(y|x)$ 
        \item $q_{\mathrm{mid}}(p^*,h^*) \triangleq \argmin_{q\in \Theta(p^*,h^*)} \sum_{x,y} \left(q(y|x) - \mathrm{Uniform}(y)\right) ^2$
    \end{itemize}
    where $\mathrm{Uniform}(y)$ indicates the uniform probability mass function for $y$.  Note that all three of these optimization problems have unique solutions and would be straightforward to solve if $p^*,h^*$ were known.  Indeed, the set $\Theta$ is a convex polytope, so the first two problems are linear programs and the last problem is a strictly convex quadratic program.  Details can be found in Appendix \ref{sec:optproblems}.  Even when $q^*$ is nonidentifiable, we can still be assured that $q_{\mathrm{lo}}, q_{\mathrm{hi}}$ are identifiable and $q_{\mathrm{lo},x,y} \leq q^*(y|x) \leq q_{\mathrm{hi},x,y}$.  We can also hope that $q_{\mathrm{mid}}$ will provide a single estimate which strikes a middle ground; among all links which are consistent with $p^*,h^*$, $q_{\mathrm{mid}}$ is the one which is closest to uniform in a sum-of-squares sense.
\end{itemize}  
...
\begin{enumerate}
    ...
    \[
    \hat q \triangleq \mathbb{E}\left[q_{\mathrm{mid}}(p^*,h^*) | N^X,N^Y,G(p^*,h^*)=0\right]
    \]
    %
    \item $C_{x,y}$ is calculated using credible intervals.  For each $x,y$, we define $C_{x,y}$ to be the largest interval such that 
    \begin{align*}
    \mathbb{P}\left(q_{\mathrm{lo},x,y}(p^*,h^*) \in C_{x,y} | N^X,N^Y,G(p^*,h^*)=0\right)\geq 97.5\% \\
    \mathbb{P}\left(q_{\mathrm{hi},x,y}(p^*,h^*) \in C_{x,y} | N^X,N^Y,G(p^*,h^*)=0\right)\geq 97.5\% 
    \end{align*}
\end{enumerate}
...
In practice, we were not able to find a way to compute the credible interval and posterior means exactly.  However, given samples from the posterior distribution it would be straightforward to get good estimates, using use monte-carlo approximations.  Unfortunately, it seems difficult to obtain samples from the posterior distribution, $\mathbb{P}(p^*,h^*| N^X,N^Y,G(p^*,h^*)=0)$.   Common approaches to this type of problem involve Markov-Chain Monte Carlo and Variational Inference, but we were unable to make these approaches work in practice.  It seems nontrivial to work with the condition $G(p,h)=0$ that formalizes the MLM assumption.  We instead take a somewhat na\"ive approach.  We start by drawing samples according to
\[
P,H ~ \sim \mathbb{P}(p^*,h^* | N^X,N^Y)
\]
This can be achieved exactly, using the the conjugacy between our prior and the Multinomial distribution.  Notice that these samples do not incorporate our knowledge of the MLM assumption, insofar as they are not conditioned on the event $G(p^*,h^*)=0$.  To approximately remedy this, we define $\tilde H$ as the solution of $\min_{h'}D(H|h')$, subject to the constraint that $E(P,h')=0$.  Optimization details can be found in Appendix \ref{sec:optproblems}.  We use the pair $P,\tilde H$ as \emph{approximate} samples for the distribution $\mathbb{P}(p^*,h^*| N^X,N^Y,G(p^*,h^*)=0)$.  We can repeat this process to produce many samples of $(P,\tilde H)$ and use those samples to produce approximate monte-carlo estimates for $\hat q(y|x),C_{x,y}$.  We asymptotically expect that $P,H$ will nearly satisfy the MLM assumption in any case, so this approximation should not make a large difference.  However, we caution that the exact implications of the approximation are difficult to reason about.   In any case, it is a practical solution to a difficult problem.
